...
function sharedAclTests(defaultSettings) {
  ...
  it(`cannot register a role with an empty name`, () => {
    expect(() => aclService.register('', null, { view: ['users'] }))
      .toThrow(new Error('Wtf2AclService: role name cannot be empty'));
  });

  it(`cannot check bulk resource`, () => {
    expect(() => aclService.can('guest', 'edit', '*'))
      .toThrow(new Error(`Wtf2AclService: cannot use empty or bulk '*' resource placeholder with 'can' method`));
  });
...
describe('acl-service', () => {
  ...
  describe('with default settings', () => {
    ...
    beforeEach(() => {
      ...
      TestBed.configureTestingModule({
        providers: [
          { provide: WTF2_SECURITY_OPTIONS_TOKEN, useValue: {} },
          Wtf2AclService,
        ],
      });
    });
  ...
  describe('with some roles settings', () => {
    ...
    beforeEach(() => {
      ...
      TestBed.configureTestingModule({
        providers: [
          { provide: WTF2_SECURITY_OPTIONS_TOKEN, useValue: defaultSettings }, // useValue will clone
          Wtf2AclService,
        ],
      });
    });
  ...
  describe('with some roles settings (not cloned)', () => {
    ...
    beforeEach(() => {
      ...
      TestBed.configureTestingModule({
        providers: [
          { provide: WTF2_SECURITY_OPTIONS_TOKEN, useFactory: () => defaultSettings },
          Wtf2AclService,
        ],
      });
    });
  ...
  describe('with bulk resources', () => {
    ...
    beforeEach(() => {
      ...
      TestBed.configureTestingModule({
        providers: [
          { provide: WTF2_SECURITY_OPTIONS_TOKEN, useFactory: () => defaultSettings }, // will provide a reference
          Wtf2AclService,
        ],
      });
    });
