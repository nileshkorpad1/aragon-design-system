...
import {Wtf2Chip, Wtf2ChipEvent, Wtf2ChipSelectionChange, Wtf2ChipsModule, Wtf2ChipList} from './index';
...
describe('Chips', () => {
  ...
  describe('Wtf2BasicChip', () => {
    ...
    beforeEach(() => {
      ...
      chipInstance = chipDebugElement.injector.get<Wtf2Chip>(Wtf2Chip);
    ...
    it('adds the `wtf2-basic-chip` class', () => {
      expect(chipNativeElement.classList).toContain('wtf2-chip');
      expect(chipNativeElement.classList).toContain('wtf2-basic-chip');
    });
  });
  ...
  describe('Wtf2Chip', () => {
    ...
    describe('basic behaviors', () => {
      ...
      it('adds the `wtf2-chip` class', () => {
        expect(chipNativeElement.classList).toContain('wtf2-chip');
      });

      it('does not add the `wtf2-basic-chip` class', () => {
        expect(chipNativeElement.classList).not.toContain('wtf2-basic-chip');
      });
      ...
      it('allows color customization', () => {
        expect(chipNativeElement.classList).toContain('wtf2-primary');

        testComponent.color = 'warn';
        fixture.detectChanges();

        expect(chipNativeElement.classList).not.toContain('wtf2-primary');
        expect(chipNativeElement.classList).toContain('wtf2-warn');
      });

      it('allows selection', () => {
        spyOn(testComponent, 'chipSelectionChange');
        expect(chipNativeElement.classList).not.toContain('wtf2-chip-selected');

        testComponent.selected = true;
        fixture.detectChanges();

        expect(chipNativeElement.classList).toContain('wtf2-chip-selected');
    ...
    describe('keyboard behavior', () => {
      ...
      describe('when selectable is true', () => {
        ...
        it('should selects/deselects the currently focused chip on SPACE', () => {
          ...
          const CHIP_SELECTED_EVENT: Wtf2ChipSelectionChange = {
            source: chipInstance,
            isUserInput: true,
            selected: true
          };

          const CHIP_DESELECTED_EVENT: Wtf2ChipSelectionChange = {
            source: chipInstance,
            isUserInput: true,
            selected: false
          };
...
@Component({
  template: `
    <wtf2-chip-list>
      <div *ngIf="shouldShow">
        <wtf2-chip [selectable]="selectable" [removable]="removable"
...
class SingleChip {
  ...
  chipFocus: (event?: Wtf2ChipEvent) => void = () => {};
  chipDestroy: (event?: Wtf2ChipEvent) => void = () => {};
  chipSelectionChange: (event?: Wtf2ChipSelectionChange) => void = () => {};
  chipRemove: (event?: Wtf2ChipEvent) => void = () => {};
}

@Component({
  template: `<wtf2-basic-chip>{{name}}</wtf2-basic-chip>`
})
class BasicChip {
}
