 ...
 * that refers to Wtf2Ripple#speedFactor.
 */
...
export class Walker extends ProgramAwareRuleWalker {
  ...
  visitBinaryExpression(expression: ts.BinaryExpression) {
    ...
    if (targetTypeName === 'Wtf2Ripple' && propertyName === 'speedFactor') {
      if (ts.isNumericLiteral(expression.right)) {
        ...
        this.addFailureAtNode(expression,
            `Found deprecated variable assignment for "${bold('Wtf2Ripple')}#${red('speedFactor')}"`,
  ...
  visitPropertyAssignment(assignment: ts.PropertyAssignment) {
    ...
    // We could technically lazily check for the WTF2_RIPPLE_GLOBAL_OPTIONS injection token to
    ...
    if (ts.isNumericLiteral(initializer)) {
      ...
      this.addFailureAtNode(assignment,
          `Found deprecated property assignment for "${bold('WTF2_RIPPLE_GLOBAL_OPTIONS')}:` +
          `${red('baseSpeedFactor')}"`,
      ...
      this.addFailureAtNode(assignment,
          `Found a deprecated property assignment for "${bold('WTF2_RIPPLE_GLOBAL_OPTIONS')}:` +
          `${red('baseSpeedFactor')}.`,
          [propertyNameReplacement, rightExpressionReplacement]);
    }
  }
