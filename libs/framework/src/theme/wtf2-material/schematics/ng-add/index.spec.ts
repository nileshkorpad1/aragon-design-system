...
import {Tree} from '@angular-devkit/schewtf2ics';
...
import {getWorkspace} from '@schewtf2ics/angular/utility/config';
import {getIndexHtmlPath} from './fonts/project-index-html';

describe('ng-add schewtf2ic', () => {
  ...
  it('should update package.json', async () => {
    // By default, the Angular workspace schewtf2ic sets up "@angular/animations". In order
    ...
    const tree = await runner.runSchewtf2icAsync('ng-add', {}, appTree).toPromise();
    ...
    expect(runner.tasks.some(task => task.name === 'run-schewtf2ic')).toBe(true);
  });

  it('should add hammerjs import to project main file', async () => {
    const tree = await runner.runSchewtf2icAsync('ng-add-setup-project', {}, appTree).toPromise();
  ...
  it('should support adding a custom theme', async () => {
    ...
    const tree = await runner.runSchewtf2icAsync('ng-add-setup-project', {theme: 'custom'}, appTree)
                     .toPromise();
    ...
    expect(themeContent).toContain(`$app-primary: wtf2-palette(`);
  });
  ...
  describe('gestures disabled', () => {
    it('should not add hammerjs to package.json', async () => {
      const tree = await runner.runSchewtf2icAsync('ng-add', {gestures: false}, appTree).toPromise();
      const packageJson = JSON.parse(getFileContent(tree, '/package.json'));
  ...
  describe('animations enabled', () => {
    ...
    it('should not add BrowserAnimationsModule if NoopAnimationsModule is set up', async () => {
      ...
      spyOn(console, 'warn');
      await runner.runSchewtf2icAsync('ng-add-setup-project', {}, appTree).toPromise();
  ...
  describe('animations disabled', () => {
    it('should add the NoopAnimationsModule to the project module', async () => {
      const tree =
          await runner.runSchewtf2icAsync('ng-add-setup-project', {animations: false}, appTree)
              .toPromise();
      const fileContent = getFileContent(tree, '/projects/material/src/app/app.module.ts');
  ...
  describe('theme files', () => {
    ...
    it('should not overwrite existing custom theme files', async () => {
      ...
      const tree =
          await runner.runSchewtf2icAsync('ng-add-setup-project', {theme: 'custom'}, appTree)
              .toPromise();
