...
import {Schewtf2icsException, Tree} from '@angular-devkit/schewtf2ics';
import {
  getProjectFromWorkspace,
  getProjectStyleFile,
  getProjectTargetOptions,
} from '@angular/cdk/schewtf2ics';
import {InsertChange} from '@schewtf2ics/angular/utility/change';
...
function insertCustomTheme(project: WorkspaceProject, projectName: string, host: Tree,
  ...
  if (!stylesPath) {
    if (!project.sourceRoot) {
      throw new Schewtf2icsException(`Could not find source root for project: "${projectName}". ` +
...
function validateDefaultTargetBuilder(project: WorkspaceProject, targetName: 'build' | 'test') {
  ...
  if (!isDefaultBuilder && targetName === 'build') {
    throw new Schewtf2icsException(`Your project is not using the default builders for ` +
      `"${targetName}". The Angular Material schewtf2ics cannot add a theme to the workspace ` +
