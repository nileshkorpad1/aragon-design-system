...
import {Wtf2RadioButton, Wtf2RadioChange, Wtf2RadioGroup, Wtf2RadioModule} from './index';

describe('Wtf2Radio', () => {

  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [Wtf2RadioModule, FormsModule, ReactiveFormsModule],
      declarations: [
  ...
  describe('inside of a group', () => {
    ...
    let groupInstance: Wtf2RadioGroup;
    let radioInstances: Wtf2RadioButton[];
    ...
    beforeEach(async(() => {
      ...
      groupDebugElement = fixture.debugElement.query(By.directive(Wtf2RadioGroup));
      groupInstance = groupDebugElement.injector.get<Wtf2RadioGroup>(Wtf2RadioGroup);

      radioDebugElements = fixture.debugElement.queryAll(By.directive(Wtf2RadioButton));
    ...
    it('should not show ripples on disabled radio buttons', () => {
      ...
      let rippleAmount = radioNativeElements[0]
          .querySelectorAll('.wtf2-ripple-element:not(.wtf2-radio-persistent-ripple)').length;
    ...
    it('should not show ripples if wtf2RippleDisabled input is set', () => {
    ...
    it('should apply class based on color attribute', () => {
      expect(radioNativeElements.every(radioEl => radioEl.classList.contains('wtf2-accent')))
        .toBe(true, 'Expected every radio element to use the accent color by default.');

      testComponent.color = 'primary';
      fixture.detectChanges();

      expect(radioNativeElements.every(radioEl => radioEl.classList.contains('wtf2-primary')))
        .toBe(true, 'Expected every radio element to use the primary color from the binding.');

      testComponent.color = 'warn';
      fixture.detectChanges();

      expect(radioNativeElements.every(radioEl => radioEl.classList.contains('wtf2-warn')))
        .toBe(true, 'Expected every radio element to use the primary color from the binding.');
    ...
    it('should have the individual button color take precedence over the group color', () => {
      ...
      expect(radioNativeElements[0].classList).toContain('wtf2-warn');
      expect(radioNativeElements[1].classList).toContain('wtf2-primary');
      expect(radioNativeElements[2].classList).toContain('wtf2-warn');
    });
  ...
  describe('group with ngModel', () => {
    ...
    it('should have the correct control state initially and after interaction', () => {
      ...
      // After changing the value programwtf2ically, the control should stay pristine
  ...
  describe('disableable', () => {
    ...
    let radioInstance: Wtf2RadioButton;
    ...
    beforeEach(() => {
      ...
      testComponent = fixture.debugElement.componentInstance;
      const radioDebugElement = fixture.debugElement.query(By.directive(Wtf2RadioButton));
      radioInstance = radioDebugElement.injector.get<Wtf2RadioButton>(Wtf2RadioButton);
  ...
  describe('as standalone', () => {
    ...
    let radioDebugElements: DebugElement[];
    let seasonRadioInstances: Wtf2RadioButton[];
    let weatherRadioInstances: Wtf2RadioButton[];
    let fruitRadioInstances: Wtf2RadioButton[];
  ...
  describe('with tabindex', () => {
    ...
    it('should forward focus to native input', () => {
      let radioButtonEl = fixture.debugElement.query(By.css('.wtf2-radio-button')).nativeElement;
      let inputEl = fixture.debugElement.query(By.css('.wtf2-radio-input')).nativeElement;
    ...
    it('should allow specifying an explicit tabindex for a single radio-button', () => {
      const radioButtonInput = fixture.debugElement
        .query(By.css('.wtf2-radio-button input')).nativeElement as HTMLInputElement;
    ...
    it('should remove the tabindex from the host element', () => {
      ...
      const radioButtonEl =
          predefinedFixture.debugElement.query(By.css('.wtf2-radio-button')).nativeElement;
...
@Component({
  template: `
  <wtf2-radio-group [disabled]="isGroupDisabled"
                  [labelPosition]="labelPos"
                  [required]="isGroupRequired"
                  [value]="groupValue"
                  name="test-name">
    <wtf2-radio-button value="fire" [disableRipple]="disableRipple" [disabled]="isFirstDisabled"
                     [color]="color">
      Charmander
    </wtf2-radio-button>
    <wtf2-radio-button value="water" [disableRipple]="disableRipple" [color]="color">
      Squirtle
    </wtf2-radio-button>
    <wtf2-radio-button value="leaf" [disableRipple]="disableRipple" [color]="color">
      Bulbasaur
    </wtf2-radio-button>
  </wtf2-radio-group>
  `
...
@Component({
  template: `
    <wtf2-radio-button name="season" value="spring">Spring</wtf2-radio-button>
    <wtf2-radio-button name="season" value="summer">Summer</wtf2-radio-button>
    <wtf2-radio-button name="season" value="autum">Autumn</wtf2-radio-button>

    <wtf2-radio-button name="weather" value="warm">Spring</wtf2-radio-button>
    <wtf2-radio-button name="weather" value="hot">Summer</wtf2-radio-button>
    <wtf2-radio-button name="weather" value="cool">Autumn</wtf2-radio-button>

    <span id="xyz">Baby Banana</span>
    <span id="abc">A smaller banana</span>
    <wtf2-radio-button name="fruit"
                     value="banana"
                     [aria-label]="ariaLabel"
                     [aria-labelledby]="ariaLabelledby"
                     [aria-describedby]="ariaDescribedby">
    </wtf2-radio-button>
    <wtf2-radio-button name="fruit" value="raspberry">Raspberry</wtf2-radio-button>
    <wtf2-radio-button id="nameless" value="no-name">No name</wtf2-radio-button>
  `
})
...
@Component({
  template: `
  <wtf2-radio-group [name]="groupName" [(ngModel)]="modelValue" (change)="lastEvent = $event">
    <wtf2-radio-button *ngFor="let option of options" [value]="option.value">
      {{option.label}}
    </wtf2-radio-button>
  </wtf2-radio-group>
  `
...
class RadioGroupWithNgModel {
  ...
  lastEvent: Wtf2RadioChange;
}

@Component({
  template: `<wtf2-radio-button>One</wtf2-radio-button>`
})
class DisableableRadioButton {
  @ViewChild(Wtf2RadioButton, {static: false}) wtf2RadioButton: Wtf2RadioButton;

  set disabled(value: boolean) {
    this.wtf2RadioButton.disabled = value;
  }
}

@Component({
  template: `
  <wtf2-radio-group [formControl]="formControl">
    <wtf2-radio-button value="1">One</wtf2-radio-button>
  </wtf2-radio-group>
  `
})
...
@Component({
  template: `<wtf2-radio-button [tabIndex]="tabIndex"></wtf2-radio-button>`
})
...
@Component({
  template: `
  <wtf2-radio-group name="group" [(ngModel)]="modelValue">
    <transcluding-wrapper *ngFor="let option of options">
      <wtf2-radio-button [value]="option.value">{{option.label}}</wtf2-radio-button>
...
@Component({
  template: `<wtf2-radio-button tabindex="0"></wtf2-radio-button>`
})
