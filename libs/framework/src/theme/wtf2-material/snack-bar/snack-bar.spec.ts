...
import {
  ...
  SimpleSnackBar,
  WTF2_SNACK_BAR_DATA,
  WTF2_SNACK_BAR_DEFAULT_OPTIONS,
} from './index';

describe('Wtf2SnackBar', () => {
  let snackBar: Wtf2SnackBar;
  ...
  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [Wtf2SnackBarModule, SnackBarTestModule, NoopAnimationsModule],
    }).compileComponents();
  }));

  beforeEach(inject([Wtf2SnackBar, LiveAnnouncer, OverlayContainer],
    (sb: Wtf2SnackBar, la: LiveAnnouncer, oc: OverlayContainer) => {
    snackBar = sb;
    liveAnnouncer = la;
    overlayContainer = oc;
  ...
  it('should open a simple message with a button', () => {
    let config: Wtf2SnackBarConfig = {viewContainerRef: testViewContainerRef};
    ...
    let buttonElement = overlayContainerElement.querySelector('button.wtf2-button')!;
  ...
  it('should open a simple message with no button', () => {
    ...
    expect(messageElement.textContent)
        .toContain(simpleMessage, `Expected the snack bar message to be '${simpleMessage}'`);
    expect(overlayContainerElement.querySelector('button.wtf2-button'))
  ...
  it('should remove snackbar if another is shown while its still aniwtf2ing open', fakeAsync(() => {
    snackBar.open('First snackbar');
    viewContainerFixture.detectChanges();
  ...
  it('should dismiss the snackbar when the action is called, notifying of both action and dismiss',
     fakeAsync(() => {
      ...
      let actionButton =
        overlayContainerElement.querySelector('button.wtf2-button') as HTMLButtonElement;
      actionButton.click();
      viewContainerFixture.detectChanges();
      tick();
  ...
  it('should dismiss autowtf2ically after a specified timeout', fakeAsync(() => {
    let config = new Wtf2SnackBarConfig();
    config.duration = 250;
  ...
  it('should be able to override the default config', fakeAsync(() => {
    ...
    TestBed
      .resetTestingModule()
      .overrideProvider(WTF2_SNACK_BAR_DEFAULT_OPTIONS, {
        deps: [],
        useFactory: () => ({panelClass: 'custom-class'})
      })
      .configureTestingModule({imports: [Wtf2SnackBarModule, NoopAnimationsModule]})
      .compileComponents();

    inject([Wtf2SnackBar, OverlayContainer], (sb: Wtf2SnackBar, oc: OverlayContainer) => {
      snackBar = sb;
      overlayContainer = oc;
      overlayContainerElement = oc.getContainerElement();
    })();
...
describe('Wtf2SnackBar with parent Wtf2SnackBar', () => {
  let parentSnackBar: Wtf2SnackBar;
  let childSnackBar: Wtf2SnackBar;
  let overlayContainer: OverlayContainer;
  let overlayContainerElement: HTMLElement;
  let fixture: ComponentFixture<ComponentThatProvidesWtf2SnackBar>;
  let liveAnnouncer: LiveAnnouncer;

  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [Wtf2SnackBarModule, SnackBarTestModule, NoopAnimationsModule],
      declarations: [ComponentThatProvidesWtf2SnackBar],
    }).compileComponents();
  }));
  ...
  beforeEach(inject([Wtf2SnackBar, LiveAnnouncer, OverlayContainer],
    ...
    fixture = TestBed.createComponent(ComponentThatProvidesWtf2SnackBar);
...
describe('Wtf2SnackBar Positioning', () => {
  let snackBar: Wtf2SnackBar;
  let liveAnnouncer: LiveAnnouncer;
  let overlayContainer: OverlayContainer;
  ...
  it('should default to bottom center', fakeAsync(() => {
    ...
    expect(containerEl.classList.contains('wtf2-snack-bar-center')).toBeTruthy();
    expect(containerEl.classList.contains('wtf2-snack-bar-top')).toBeFalsy();
  ...
  it('should be in the bottom left corner', fakeAsync(() => {
    ...
    expect(containerEl.classList.contains('wtf2-snack-bar-center')).toBeFalsy();
    expect(containerEl.classList.contains('wtf2-snack-bar-top')).toBeFalsy();
    expect(overlayPaneEl.style.marginBottom).toBe('0px', 'Expected margin-bottom to be "0px"');
    expect(overlayPaneEl.style.marginTop).toBe('', 'Expected margin-top to be ""');
    expect(overlayPaneEl.style.marginRight).toBe('', 'Expected margin-right to be ""');
   ...
   it('should be in the top left corner', fakeAsync(() => {
    ...
    expect(containerEl.classList.contains('wtf2-snack-bar-center')).toBeFalsy();
    expect(containerEl.classList.contains('wtf2-snack-bar-top')).toBeTruthy();
...
class BurritosNotification {
  constructor(
    public snackBarRef: Wtf2SnackBarRef<BurritosNotification>,
    @Inject(WTF2_SNACK_BAR_DATA) public data: any) { }
}


@Component({
  template: '',
  providers: [Wtf2SnackBar]
})
class ComponentThatProvidesWtf2SnackBar {
  constructor(public snackBar: Wtf2SnackBar) {}
}
...
@NgModule({
  imports: [CommonModule, Wtf2SnackBarModule],
