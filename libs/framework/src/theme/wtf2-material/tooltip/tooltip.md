...
To add a delay before showing or hiding the tooltip, you can use the inputs `wtf2TooltipShowDelay`
and `wtf2TooltipHideDelay` to provide a delay time in milliseconds.
...
your options using the `WTF2_TOOLTIP_DEFAULT_OPTIONS` injection token.
...
To completely disable a tooltip, set `wtf2TooltipDisabled`. While disabled, a tooltip will never be
shown.

### Accessibility

Elements with the `wtf2Tooltip` will add an `aria-describedby` label that provides a reference
