...
import {MENU_PANEL_TOP_PADDING, WTF2_MENU_SCROLL_STRATEGY} from './menu-trigger';
...
describe('Wtf2Menu', () => {
  ...
  function createComponent<T>(component: Type<T>,
    ...
    TestBed.configureTestingModule({
      imports: [Wtf2MenuModule, NoopAnimationsModule],
  ...
  it('should restore focus to the trigger when the menu was opened by keyboard', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('.wtf2-menu-panel')).toBeTruthy();
  ...
  it('should scroll the panel to the top on open, when it is scrollable', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('.wtf2-menu-panel')!.scrollTop).toBe(0);
  }));
  ...
  it('should close the menu when pressing ESCAPE', fakeAsync(() => {
    ...
    const panel = overlayContainerElement.querySelector('.wtf2-menu-panel')!;
  ...
  it('should transfer any custom classes from the host to the overlay', () => {
    ...
    const menuEl = fixture.debugElement.query(By.css('wtf2-menu')).nativeElement;
    const panel = overlayContainerElement.querySelector('.wtf2-menu-panel')!;
  ...
  it('should not remove wtf2-elevation class from overlay when panelClass is changed', () => {
    const fixture = createComponent(SimpleMenu, [], [FakeIcon]);
    ...
    expect(panel.classList).toContain('custom-one');
    expect(panel.classList).toContain('wtf2-elevation-z4');
    ...
    expect(panel.classList)
        .toContain('wtf2-elevation-z4', 'Expected wtf2-elevation-z4 not to be removed');
  });
  ...
  it('should set the "menu" role on the overlay panel', () => {
    ...
    const menuPanel = overlayContainerElement.querySelector('.wtf2-menu-panel');
  ...
  it('should set the "menuitem" role on the items by default', () => {
    ...
    const items = Array.from(overlayContainerElement.querySelectorAll('.wtf2-menu-item'));
  ...
  it('should close the menu when using the CloseScrollStrategy', fakeAsync(() => {
    ...
    const fixture = createComponent(SimpleMenu,  [
      ...
      {
        provide: WTF2_MENU_SCROLL_STRATEGY,
        deps: [Overlay],
  ...
  it('should switch to keyboard focus when using the keyboard after opening using the mouse',
    fakeAsync(() => {
      ...
      const panel = document.querySelector('.wtf2-menu-panel')! as HTMLElement;
      const items: HTMLElement[] =
          Array.from(panel.querySelectorAll('.wtf2-menu-panel [wtf2-menu-item]'));
  ...
  it('should throw the correct error if the menu is not defined after init', () => {
    ...
    expect(() => {
      fixture.componentInstance.trigger.openMenu();
      fixture.detectChanges();
    }).toThrowError(/must pass in an wtf2-menu instance/);
  });
  ...
  it('should focus the first item when pressing home', fakeAsync(() => {
    ...
    const panel = overlayContainerElement.querySelector('.wtf2-menu-panel')!;
    const items = Array.from(panel.querySelectorAll('.wtf2-menu-item')) as HTMLElement[];
  ...
  describe('lazy rendering', () => {
    ...
    it('should focus the first menu item when opening a lazy menu via keyboard', fakeAsync(() => {
      ...
      const item = document.querySelector('.wtf2-menu-panel [wtf2-menu-item]')!;
    ...
    it('should be able to open the same menu with a different context', fakeAsync(() => {
      ...
      let item = overlayContainerElement.querySelector('.wtf2-menu-panel [wtf2-menu-item]')!;
      ...
      fixture.detectChanges();
      tick(500);
      item = overlayContainerElement.querySelector('.wtf2-menu-panel [wtf2-menu-item]')!;
  ...
  describe('positions', () => {
    ...
    it('should append wtf2-menu-before if the x position is changed', () => {
      fixture.componentInstance.trigger.openMenu();
      fixture.detectChanges();

      const panel = overlayContainerElement.querySelector('.wtf2-menu-panel') as HTMLElement;

      expect(panel.classList).toContain('wtf2-menu-before');
      expect(panel.classList).not.toContain('wtf2-menu-after');

      fixture.componentInstance.xPosition = 'after';
      fixture.detectChanges();

      expect(panel.classList).toContain('wtf2-menu-after');
      expect(panel.classList).not.toContain('wtf2-menu-before');
    });

    it('should append wtf2-menu-above if the y position is changed', () => {
      fixture.componentInstance.trigger.openMenu();
      fixture.detectChanges();

      const panel = overlayContainerElement.querySelector('.wtf2-menu-panel') as HTMLElement;

      expect(panel.classList).toContain('wtf2-menu-above');
      expect(panel.classList).not.toContain('wtf2-menu-below');

      fixture.componentInstance.yPosition = 'below';
      fixture.detectChanges();

      expect(panel.classList).toContain('wtf2-menu-below');
      expect(panel.classList).not.toContain('wtf2-menu-above');
    });
    ...
    it('should be able to update the position after the first open', () => {
      ...
      let panel = overlayContainerElement.querySelector('.wtf2-menu-panel') as HTMLElement;
      ...
      fixture.componentInstance.trigger.openMenu();
      fixture.detectChanges();
      panel = overlayContainerElement.querySelector('.wtf2-menu-panel') as HTMLElement;
  ...
  describe('overlapping trigger', () => {
    ...
    class OverlapSubject<T extends TestableMenu> {
      ...
      get menuPanel() {
        return overlayContainerElement.querySelector('.wtf2-menu-panel');
      }
    ...
    describe('not overlapping', () => {
      ...
      it('repositions the origin to be below, so the menu opens from the trigger', () => {
        ...
        expect(subject.menuPanel!.classList).toContain('wtf2-menu-below');
        expect(subject.menuPanel!.classList).not.toContain('wtf2-menu-above');
      });
    });
  });
  ...
  describe('animations', () => {
    it('should enable ripples on items by default', () => {
      ...
      const item = fixture.debugElement.query(By.css('.wtf2-menu-item'));
      const ripple = item.query(By.css('.wtf2-ripple')).injector.get<Wtf2Ripple>(Wtf2Ripple);
    ...
    it('should disable ripples on disabled items', () => {
      ...
      const items = fixture.debugElement.queryAll(By.css('.wtf2-menu-item'));
      const ripple = items[1].query(By.css('.wtf2-ripple')).injector.get<Wtf2Ripple>(Wtf2Ripple);
    ...
    it('should disable ripples if disableRipple is set', () => {
      ...
      const items = fixture.debugElement.queryAll(By.css('.wtf2-menu-item'));
      const ripple = items[2].query(By.css('.wtf2-ripple')).injector.get<Wtf2Ripple>(Wtf2Ripple);
  ...
  describe('close event', () => {
    ...
    it('should emit an event when a menu item is clicked', () => {
      const menuItem = overlayContainerElement.querySelector('[wtf2-menu-item]') as HTMLElement;
    ...
    it('should emit an event when pressing ESCAPE', () => {
      const menu = overlayContainerElement.querySelector('.wtf2-menu-panel') as HTMLElement;
  ...
  describe('nested menu', () => {
    ...
    it('should emit an event when the hover state of the menu items changes', () => {
      ...
      const menuItems = overlay.querySelectorAll('[wtf2-menu-item]');
    ...
    it('should toggle a nested menu when its trigger is hovered', fakeAsync(() => {
      ...
      instance.rootTriggerEl.nativeElement.click();
      fixture.detectChanges();
      expect(overlay.querySelectorAll('.wtf2-menu-panel').length).toBe(1, 'Expected one open menu');

      const items = Array.from(overlay.querySelectorAll('.wtf2-menu-panel [wtf2-menu-item]'));
      ...
      expect(levelOneTrigger.classList)
          .toContain('wtf2-menu-item-highlighted', 'Expected the trigger to be highlighted');
      expect(overlay.querySelectorAll('.wtf2-menu-panel').length).toBe(2, 'Expected two open menus');
      ...
      expect(overlay.querySelectorAll('.wtf2-menu-panel').length).toBe(1, 'Expected one open menu');
      expect(levelOneTrigger.classList)
          .not.toContain('wtf2-menu-item-highlighted', 'Expected the trigger to not be highlighted');
    }));
    ...
    it('should close all the open sub-menus when the hover state is changed at the root',
      fakeAsync(() => {
        ...
        expect(overlay.querySelectorAll('.wtf2-menu-panel').length)
      ...
      it('should close submenu when hovering over disabled sibling item', fakeAsync(() => {
        ...
        const items = fixture.debugElement.queryAll(By.directive(Wtf2MenuItem));
    ...
    it('should not open submenu when hovering over disabled trigger', fakeAsync(() => {
      ...
      const item = fixture.debugElement.query(By.directive(Wtf2MenuItem));
    ...
    it('should open and close a nested menu with arrow keys in ltr', fakeAsync(() => {
      ...
      const panels = overlay.querySelectorAll('.wtf2-menu-panel');
      ...
      expect(overlay.querySelectorAll('.wtf2-menu-panel').length).toBe(1);
    }));
    ...
    it('should not do anything with the arrow keys for a top-level menu', () => {
      ...
      const menu = overlay.querySelector('.wtf2-menu-panel')!;
    ...
    it('should close all of the menus when the backdrop is clicked', fakeAsync(() => {
      ...
      expect(overlay.querySelectorAll('.wtf2-menu-panel, .cdk-overlay-backdrop')[0].classList)
      ...
      expect(overlay.querySelectorAll('.wtf2-menu-panel').length).toBe(0, 'Expected no open menus');
    }));
    ...
    it('should shift focus between the sub-menus', () => {
      ...
      expect(overlay.querySelector('.wtf2-menu-panel')!.contains(document.activeElement))
          .toBe(true, 'Expected focus to be inside the root menu');

      instance.levelOneTrigger.openMenu();
      fixture.detectChanges();

      expect(overlay.querySelectorAll('.wtf2-menu-panel')[1].contains(document.activeElement))
          .toBe(true, 'Expected focus to be inside the first nested menu');

      instance.levelTwoTrigger.openMenu();
      fixture.detectChanges();

      expect(overlay.querySelectorAll('.wtf2-menu-panel')[2].contains(document.activeElement))
    ...
    it('should close all of the menus when an item is clicked', fakeAsync(() => {
      ...
      const menus = overlay.querySelectorAll('.wtf2-menu-panel');

      expect(menus.length).toBe(3, 'Expected three open menus');

      (menus[2].querySelector('.wtf2-menu-item')! as HTMLElement).click();
      fixture.detectChanges();
      tick(500);
    ...
    it('should set a class on the menu items that trigger a sub-menu', () => {
      ...
      expect(menuItems[0].classList).toContain('wtf2-menu-item-submenu-trigger');
      expect(menuItems[1].classList).not.toContain('wtf2-menu-item-submenu-trigger');
    });
    ...
    it('should increase the sub-menu elevation based on its depth', () => {
      ...
      expect(menus[0].classList)
          .toContain('wtf2-elevation-z4', 'Expected root menu to have base elevation.');
      expect(menus[1].classList)
          .toContain('wtf2-elevation-z5', 'Expected first sub-menu to have base elevation + 1.');
      expect(menus[2].classList)
          .toContain('wtf2-elevation-z6', 'Expected second sub-menu to have base elevation + 2.');
    });
    ...
    it('should update the elevation when the same menu is opened at a different depth',
      fakeAsync(() => {
        ...
        let lastMenu = overlay.querySelectorAll('.wtf2-menu-panel')[2];

        expect(lastMenu.classList)
            .toContain('wtf2-elevation-z6', 'Expected menu to have the base elevation plus two.');
        ...
        lastMenu = overlay.querySelector('.wtf2-menu-panel') as HTMLElement;

        expect(lastMenu.classList)
            .not.toContain('wtf2-elevation-z6', 'Expected menu not to maintain old elevation.');
        expect(lastMenu.classList)
            .toContain('wtf2-elevation-z4', 'Expected menu to have the proper updated elevation.');
      }));
    ...
    it('should not increase the elevation if the user specified a custom one', () => {
      ...
      const menuClasses = overlayContainerElement.querySelectorAll('.wtf2-menu-panel')[1].classList;

      expect(menuClasses)
          .toContain('wtf2-elevation-z24', 'Expected user elevation to be maintained');
      expect(menuClasses)
          .not.toContain('wtf2-elevation-z3', 'Expected no stacked elevation.');
    });

    it('should close all of the menus when the root is closed programwtf2ically', fakeAsync(() => {
      compileTestComponent();
      instance.rootTrigger.openMenu();
      fixture.detectChanges();
    ...
    it('should prevent the default mousedown action if the menu item opens a sub-menu', () => {
      ...
      dispatchMouseEvent(overlay.querySelector('[wtf2-menu-item]')!, 'mousedown', 0, 0, event);
    ...
    it('should close the initial menu if the user moves away while aniwtf2ing', fakeAsync(() => {
      const repeaterFixture = createComponent(NestedMenuRepeater);
      overlay = overlayContainerElement;
...
describe('Wtf2Menu default overrides', () => {
  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [Wtf2MenuModule, NoopAnimationsModule],
      declarations: [SimpleMenu, FakeIcon],
      providers: [{
        provide: WTF2_MENU_DEFAULT_OPTIONS,
...
@Component({
  template: `
    <button
      [wtf2MenuTriggerFor]="menu"
      [wtf2MenuTriggerRestoreFocus]="restoreFocus"
    ...
    <wtf2-menu
      ...
      <button wtf2-menu-item> Item </button>
      <button wtf2-menu-item disabled> Disabled </button>
      <button wtf2-menu-item disableRipple>
        <fake-icon>unicorn</fake-icon>
        Item with an icon
      </button>
      <button *ngFor="let item of extraItems" wtf2-menu-item> {{item}} </button>
    </wtf2-menu>
  `
})
class SimpleMenu {
  @ViewChild(Wtf2MenuTrigger, {static: false}) trigger: Wtf2MenuTrigger;
  @ViewChild('triggerEl', {static: false}) triggerEl: ElementRef<HTMLElement>;
  @ViewChild(Wtf2Menu, {static: false}) menu: Wtf2Menu;
  @ViewChildren(Wtf2MenuItem) items: QueryList<Wtf2MenuItem>;
  extraItems: string[] = [];
...
@Component({
  template: `
    <button [wtf2MenuTriggerFor]="menu" #triggerEl>Toggle menu</button>
    <wtf2-menu [xPosition]="xPosition" [yPosition]="yPosition" #menu="wtf2Menu">
      <button wtf2-menu-item> Positioned Content </button>
    </wtf2-menu>
  `
})
...
interface TestableMenu {
  trigger: Wtf2MenuTrigger;
  triggerEl: ElementRef<HTMLElement>;
}
@Component({
  template: `
    <button [wtf2MenuTriggerFor]="menu" #triggerEl>Toggle menu</button>
    <wtf2-menu [overlapTrigger]="overlapTrigger" #menu="wtf2Menu">
      <button wtf2-menu-item> Not overlapped Content </button>
    </wtf2-menu>
  `
})
...
@Component({
  ...
  `,
  exportAs: 'wtf2CustomMenu'
})
class CustomMenuPanel implements Wtf2MenuPanel {
  direction: Direction;
  xPosition: MenuPositionX = 'after';
  yPosition: MenuPositionY = 'below';
  overlapTrigger = true;
  parentMenu: Wtf2MenuPanel;
...
@Component({
  template: `
    <button [wtf2MenuTriggerFor]="menu">Toggle menu</button>
    <custom-menu #menu="wtf2CustomMenu">
      <button wtf2-menu-item> Custom Content </button>
    </custom-menu>
  `
})
class CustomMenu {
...
@Component({
  template: `
    <button
      [wtf2MenuTriggerFor]="root"
      #rootTrigger="wtf2MenuTrigger"
      #rootTriggerEl>Toggle menu</button>

    <button
      [wtf2MenuTriggerFor]="levelTwo"
      #alternateTrigger="wtf2MenuTrigger">Toggle alternate menu</button>

    <wtf2-menu #root="wtf2Menu" (closed)="rootCloseCallback($event)">
      <button wtf2-menu-item
        id="level-one-trigger"
        [wtf2MenuTriggerFor]="levelOne"
        #levelOneTrigger="wtf2MenuTrigger">One</button>
      <button wtf2-menu-item>Two</button>
      <button wtf2-menu-item
        *ngIf="showLazy"
        id="lazy-trigger"
        [wtf2MenuTriggerFor]="lazy"
        #lazyTrigger="wtf2MenuTrigger">Three</button>
    </wtf2-menu>

    <wtf2-menu #levelOne="wtf2Menu" (closed)="levelOneCloseCallback($event)">
      <button wtf2-menu-item>Four</button>
      <button wtf2-menu-item
        id="level-two-trigger"
        [wtf2MenuTriggerFor]="levelTwo"
        #levelTwoTrigger="wtf2MenuTrigger">Five</button>
      <button wtf2-menu-item>Six</button>
    </wtf2-menu>

    <wtf2-menu #levelTwo="wtf2Menu" (closed)="levelTwoCloseCallback($event)">
      <button wtf2-menu-item>Seven</button>
      <button wtf2-menu-item>Eight</button>
      <button wtf2-menu-item>Nine</button>
    </wtf2-menu>

    <wtf2-menu #lazy="wtf2Menu">
      <button wtf2-menu-item>Ten</button>
      <button wtf2-menu-item>Eleven</button>
      <button wtf2-menu-item>Twelve</button>
    </wtf2-menu>
  `
})
class NestedMenu {
  @ViewChild('root', {static: false}) rootMenu: Wtf2Menu;
  @ViewChild('rootTrigger', {static: false}) rootTrigger: Wtf2MenuTrigger;
  @ViewChild('rootTriggerEl', {static: false}) rootTriggerEl: ElementRef<HTMLElement>;
  @ViewChild('alternateTrigger', {static: false}) alternateTrigger: Wtf2MenuTrigger;
  readonly rootCloseCallback = jasmine.createSpy('root menu closed callback');

  @ViewChild('levelOne', {static: false}) levelOneMenu: Wtf2Menu;
  @ViewChild('levelOneTrigger', {static: false}) levelOneTrigger: Wtf2MenuTrigger;
  readonly levelOneCloseCallback = jasmine.createSpy('level one menu closed callback');

  @ViewChild('levelTwo', {static: false}) levelTwoMenu: Wtf2Menu;
  @ViewChild('levelTwoTrigger', {static: false}) levelTwoTrigger: Wtf2MenuTrigger;
  readonly levelTwoCloseCallback = jasmine.createSpy('level one menu closed callback');

  @ViewChild('lazy', {static: false}) lazyMenu: Wtf2Menu;
  @ViewChild('lazyTrigger', {static: false}) lazyTrigger: Wtf2MenuTrigger;
  showLazy = false;
}

@Component({
  template: `
    <button [wtf2MenuTriggerFor]="root" #rootTrigger="wtf2MenuTrigger">Toggle menu</button>

    <wtf2-menu #root="wtf2Menu">
      <button wtf2-menu-item
        [wtf2MenuTriggerFor]="levelOne"
        #levelOneTrigger="wtf2MenuTrigger">One</button>
    </wtf2-menu>

    <wtf2-menu #levelOne="wtf2Menu" class="wtf2-elevation-z24">
      <button wtf2-menu-item>Two</button>
    </wtf2-menu>
  `
})
...
@Component({
  template: `
    <button [wtf2MenuTriggerFor]="root" #rootTriggerEl>Toggle menu</button>
    <wtf2-menu #root="wtf2Menu">
      <button
        wtf2-menu-item
        class="level-one-trigger"
        *ngFor="let item of items"
        [wtf2MenuTriggerFor]="levelOne">{{item}}</button>
    </wtf2-menu>

    <wtf2-menu #levelOne="wtf2Menu">
      <button wtf2-menu-item>Four</button>
      <button wtf2-menu-item>Five</button>
    </wtf2-menu>
  `
})
...
@Component({
  template: `
    ...
    <wtf2-menu #root="wtf2Menu">
      <button wtf2-menu-item class="level-one-trigger" [wtf2MenuTriggerFor]="levelOne">One</button>

      <wtf2-menu #levelOne="wtf2Menu">
        <button wtf2-menu-item class="level-two-item">Two</button>
      </wtf2-menu>
    </wtf2-menu>
  `
})
  ...
  template: `
    ...
    <wtf2-menu #menu="wtf2Menu">
      <ng-template wtf2MenuContent>
        <button wtf2-menu-item>Item</button>
        <button wtf2-menu-item>Another item</button>
      </ng-template>
    </wtf2-menu>
  `
})
...
@Component({
  template: `
    <button
      [wtf2MenuTriggerFor]="menu"
      [wtf2MenuTriggerData]="{label: 'one'}"
      #triggerOne="wtf2MenuTrigger">One</button>

    <button
      [wtf2MenuTriggerFor]="menu"
      [wtf2MenuTriggerData]="{label: 'two'}"
      #triggerTwo="wtf2MenuTrigger">Two</button>

    <wtf2-menu #menu="wtf2Menu">
      <ng-template let-label="label" wtf2MenuContent>
        <button wtf2-menu-item>{{label}}</button>
      </ng-template>
    </wtf2-menu>
  `
})
class LazyMenuWithContext {
  @ViewChild('triggerOne', {static: false}) triggerOne: Wtf2MenuTrigger;
  @ViewChild('triggerTwo', {static: false}) triggerTwo: Wtf2MenuTrigger;
}
...
@Component({
  template: `
    <button [wtf2MenuTriggerFor]="one">Toggle menu</button>
    <wtf2-menu #one="wtf2Menu">
      <button wtf2-menu-item>One</button>
    </wtf2-menu>

    <wtf2-menu #two="wtf2Menu">
      <button wtf2-menu-item>Two</button>
    </wtf2-menu>
  `
})
class DynamicPanelMenu {
  @ViewChild(Wtf2MenuTrigger, {static: false}) trigger: Wtf2MenuTrigger;
  @ViewChild('one', {static: false}) firstMenu: Wtf2Menu;
  @ViewChild('two', {static: false}) secondMenu: Wtf2Menu;
}
...
@Component({
  template: `
    ...
    <wtf2-menu #menu="wtf2Menu">
      <button wtf2-menu-item role="menuitemcheckbox" aria-checked="true">Checked</button>
      <button wtf2-menu-item role="menuitemcheckbox" aria-checked="false">Not checked</button>
    </wtf2-menu>
  `
})
