...
describe('menu', () => {
  const menuSelector = '.wtf2-menu-panel';
  const page = {
    ...
    items: (index: number) => element.all(by.css('[wtf2-menu-item]')).get(index),
  ...
  it('should mirror classes on host to menu template in overlay', async () => {
    await page.trigger().click();
    expect(await page.menu().getAttribute('class')).toContain('wtf2-menu-panel');
