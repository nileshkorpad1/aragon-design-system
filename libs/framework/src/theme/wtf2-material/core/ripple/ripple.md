...
The `wtf2Ripple` attribute directive defines an area in which a ripple animates on user interaction.

```html
<div wtf2Ripple [wtf2RippleColor]="myColor">
  <ng-content></ng-content>
</div>
```

By default, a ripple is activated when the host element of the `wtf2Ripple` directive receives
...
Ripples can also be triggered programwtf2ically by getting a reference to the Wtf2Ripple directive
...
the `wtf2RippleTrigger` option that expects a reference to an `HTMLElement`.

```html
<div>
  <div wtf2Ripple [wtf2RippleTrigger]="trigger" class="my-ripple-container">
...
Ripples can be shown programwtf2ically by getting a reference to the `Wtf2Ripple` directive.

```ts
class MyComponent {

  /** Reference to the directive instance of the ripple. */
  @ViewChild(Wtf2Ripple) ripple: Wtf2Ripple;
...
ripple option has been set to `true` and the coordinates would not wtf2ter.

Ripples that are being dispatched programwtf2ically can be launched with the `persistent` option.
This means that the ripples will not fade out autowtf2ically, and need to be faded out using
...
Global ripple options can be specified by setting the `WTF2_RIPPLE_GLOBAL_OPTIONS` provider.
...
@NgModule({
  providers: [
    {provide: WTF2_RIPPLE_GLOBAL_OPTIONS, useValue: globalRippleConfig}
  ]
})
```
...
To change global ripple options at runtime, just inject the `WTF2_RIPPLE_GLOBAL_OPTIONS`
...
@NgModule({
  providers: [
    {provide: WTF2_RIPPLE_GLOBAL_OPTIONS, useExisting: AppGlobalRippleOptions},
  ]
})
