...
import {WTF2_RIPPLE_GLOBAL_OPTIONS, RippleGlobalOptions} from '../core';
import {By} from '@angular/platform-browser';
import {dispatchFakeEvent, dispatchMouseEvent} from '@angular/cdk/testing';
import {Direction, Directionality} from '@angular/cdk/bidi';
import {Subject} from 'rxjs';
import {Wtf2TabLink, Wtf2TabNav, Wtf2TabsModule} from '../index';


describe('Wtf2TabNavBar', () => {
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      ...
      providers: [
        {provide: WTF2_RIPPLE_GLOBAL_OPTIONS, useFactory: () => globalRippleOptions},
  ...
  describe('basic behavior', () => {
    ...
    it('should add the active class if active', () => {
      ...
      fixture.detectChanges();
      expect(tabLinkElements[0].classList.contains('wtf2-tab-label-active')).toBeTruthy();
      expect(tabLinkElements[1].classList.contains('wtf2-tab-label-active')).toBeFalsy();

      tabLink2.nativeElement.click();
      fixture.detectChanges();
      expect(tabLinkElements[0].classList.contains('wtf2-tab-label-active')).toBeFalsy();
      expect(tabLinkElements[1].classList.contains('wtf2-tab-label-active')).toBeTruthy();
    });
    ...
    it('should add the disabled class if disabled', () => {
      ...
      expect(tabLinkElements.every(tabLinkEl => !tabLinkEl.classList.contains('wtf2-tab-disabled')))
  ...
  it('should clean up the ripple event handlers on destroy', () => {
    ...
    let link = fixture.debugElement.nativeElement.querySelector('.wtf2-tab-link');
    ...
    expect(link.querySelector('.wtf2-ripple-element'))
  ...
  it('should support the native tabindex attribute', () => {
    ...
    const tabLink = fixture.debugElement.query(By.directive(Wtf2TabLink))
        .injector.get<Wtf2TabLink>(Wtf2TabLink);
  ...
  describe('ripples', () => {
    ...
    it('should show up for tab link elements on mousedown', () => {
      const tabLink = fixture.debugElement.nativeElement.querySelector('.wtf2-tab-link');

      dispatchMouseEvent(tabLink, 'mousedown');
      dispatchMouseEvent(tabLink, 'mouseup');

      expect(tabLink.querySelectorAll('.wtf2-ripple-element').length)
    ...
    it('should be able to disable ripples on an individual tab link', () => {
      ...
      const tabLinkInstance = tabLinkDebug.injector.get<Wtf2TabLink>(Wtf2TabLink);
      ...
      expect(tabLinkElement.querySelectorAll('.wtf2-ripple-element').length)
...
@Component({
  selector: 'test-app',
  template: `
    <nav wtf2-tab-nav-bar [disableRipple]="disableRippleOnBar">
      <a wtf2-tab-link
...
class SimpleTabNavBarTestApp {
  @ViewChild(Wtf2TabNav, {static: false}) tabNavBar: Wtf2TabNav;
  @ViewChildren(Wtf2TabLink) tabLinks: QueryList<Wtf2TabLink>;
...
@Component({
  template: `
    <nav wtf2-tab-nav-bar>
      <a wtf2-tab-link *ngIf="!isDestroyed">Link</a>
    </nav>
  `
})
...
@Component({
  template: `
    <nav wtf2-tab-nav-bar>
      <a wtf2-tab-link [tabIndex]="tabIndex">TabIndex Link</a>
    </nav>
  `
})
...
@Component({
  template: `
    <nav wtf2-tab-nav-bar>
      <a wtf2-tab-link tabindex="5">Link</a>
    </nav>
  `
})
