...
describe('tabs', () => {
  describe('basic behavior', () => {
    ...
    beforeEach(async () => {
      ...
      tabLabels = element.all(by.css('.wtf2-tab-label'));
      tabBodies = element.all(by.css('wtf2-tab-body'));
    });
...
function getLabelActiveStates(elements: ElementArrayFinder) {
  return getClassStates(elements, 'wtf2-tab-label-active');
}

/** Returns an array of true/false that represents the active states for the provided elements */
function getBodyActiveStates(elements: ElementArrayFinder) {
  return getClassStates(elements, 'wtf2-tab-body-active');
}
