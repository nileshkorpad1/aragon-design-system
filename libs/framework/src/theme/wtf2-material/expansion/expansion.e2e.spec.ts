...
describe('expansion', () => {
  ...
  it('should show an accordion', async () => {
    expect(await element(by.css('.wtf2-accordion'))).toBeDefined();
  });

  it('should show two panels', async () => {
    expect(await element.all(by.css('.wtf2-expansion-panel')).count()).toBe(2);
  });

  it('should hide contents of the expansion panel on click', async () => {
    const panelHeader = element.all(by.css('.wtf2-expansion-panel-header')).get(0);
    const panelContent = element.all(by.css('.wtf2-expansion-panel-content')).get(0);
  ...
  it('should emit events for expanding and collapsing', async () => {
    const panelHeader = element.all(by.css('.wtf2-expansion-panel-header')).get(1);
    const panelDescription = element
      .all(by.css('.wtf2-expansion-panel-header wtf2-panel-description')).get(1);
