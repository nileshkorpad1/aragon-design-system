...
import {
  ...
  Wtf2DialogModule,
  Wtf2DialogRef,
  WTF2_DIALOG_DEFAULT_OPTIONS
} from './index';
import {Subject} from 'rxjs';


describe('Wtf2Dialog', () => {
  let dialog: Wtf2Dialog;
  ...
  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [Wtf2DialogModule, DialogTestModule],
      providers: [
  ...
  beforeEach(inject([Wtf2Dialog, Location, OverlayContainer],
    (d: Wtf2Dialog, l: Location, oc: OverlayContainer) => {
      dialog = d;
  ...
  it('should open a dialog with a component', () => {
    ...
    viewContainerFixture.detectChanges();
    let dialogContainerElement = overlayContainerElement.querySelector('wtf2-dialog-container')!;
  ...
  it('should close a dialog and get back a result', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('wtf2-dialog-container')).toBeNull();
  }));
  ...
  it('should close a dialog and get back a result before it is closed', fakeAsync(() => {
    ...
    const beforeCloseHandler = jasmine.createSpy('beforeClose callback').and.callFake(() => {
      expect(overlayContainerElement.querySelector('wtf2-dialog-container'))
  ...
  it('should not close a dialog via the escape key with a modifier', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('wtf2-dialog-container')).toBeTruthy();
  ...
  it('should close from a ViewContainerRef with OnPush change detection', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('wtf2-dialog-container').length)
  ...
  it('should close when clicking on the overlay backdrop', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelector('wtf2-dialog-container')).toBeFalsy();
  }));
  ...
  it('should emit the keyboardEvent stream when key events target the overlay', fakeAsync(() => {
    ...
    let backdrop = overlayContainerElement.querySelector('.cdk-overlay-backdrop') as HTMLElement;
    let container = overlayContainerElement.querySelector('wtf2-dialog-container') as HTMLElement;
  ...
  it('should close all of the dialogs', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('wtf2-dialog-container').length).toBe(3);

    dialog.closeAll();
    viewContainerFixture.detectChanges();
    flush();

    expect(overlayContainerElement.querySelectorAll('wtf2-dialog-container').length).toBe(0);
  }));

  it('should set the proper animation states', () => {
    let dialogRef = dialog.open(PizzaMsg, { viewContainerRef: testViewContainerRef });
    let dialogContainer: Wtf2DialogContainer =
        viewContainerFixture.debugElement.query(By.directive(Wtf2DialogContainer)).componentInstance;
  ...
  it('should close all dialogs when the user goes forwards/backwards in history', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('wtf2-dialog-container').length).toBe(2);
  ...
  it('should allow the consumer to disable closing a dialog on navigation', fakeAsync(() => {
    ...
    expect(overlayContainerElement.querySelectorAll('wtf2-dialog-container').length).toBe(1);
  }));
  ...
  describe('focus management', () => {
    ...
    it('should move focus to the container if there are no focusable elements in the dialog',
      fakeAsync(() => {
        ...
        expect(document.activeElement!.tagName)
            .toBe('wtf2-DIALOG-CONTAINER', 'Expected dialog container to be focused.');
      }));
  ...
  describe('dialog content elements', () => {
    let dialogRef: Wtf2DialogRef<any>;
    ...
    function runContentElementTests() {
      it('should close the dialog when clicking on the close button', fakeAsync(() => {
        ...
        (overlayContainerElement.querySelector('button[wtf2-dialog-close]') as HTMLElement).click();
        viewContainerFixture.detectChanges();
        flush();
      ...
      it('should not close if [wtf2-dialog-close] is applied on a non-button node', () => {
        expect(overlayContainerElement.querySelectorAll('.wtf2-dialog-container').length).toBe(1);

        (overlayContainerElement.querySelector('div[wtf2-dialog-close]') as HTMLElement).click();
      ...
      it('should override the "type" attribute of the close button', () => {
        let button = overlayContainerElement.querySelector('button[wtf2-dialog-close]')!;

        expect(button.getAttribute('type')).toBe('button');
      });

      it('should return the [wtf2-dialog-close] result when clicking the close button',
        fakeAsync(() => {
          let afterCloseCallback = jasmine.createSpy('afterClose callback');
          dialogRef.afterClosed().subscribe(afterCloseCallback);
      ...
      it('should set the aria-labelledby attribute to the id of the title', fakeAsync(() => {
        let title = overlayContainerElement.querySelector('[wtf2-dialog-title]')!;
        let container = overlayContainerElement.querySelector('wtf2-dialog-container')!;
  ...
  describe('aria-labelledby', () => {
    it('should be able to set a custom aria-labelledby', () => {
      ...
      const container = overlayContainerElement.querySelector('wtf2-dialog-container')!;
      expect(container.getAttribute('aria-labelledby')).toBe('Labelled By');
    });

    it('should not set the aria-labelledby autowtf2ically if it has an aria-label ' +
    ...
    it('should set the aria-labelledby attribute to the config provided aria-labelledby ' +
      'instead of the wtf2-dialog-title id', fakeAsync(() => {
        dialog.open(ContentElementDialog, {
          ariaLabelledBy: 'Labelled By',
          viewContainerRef: testViewContainerRef
        });
  ...
  describe('aria-label', () => {
    ...
    it('should not set the aria-labelledby autowtf2ically if it has an aria-label', fakeAsync(() => {
      dialog.open(ContentElementDialog, {
        ariaLabel: 'Hello there',
        viewContainerRef: testViewContainerRef
      });
...
describe('Wtf2Dialog with a parent Wtf2Dialog', () => {
  let parentDialog: Wtf2Dialog;
  let childDialog: Wtf2Dialog;
  let overlayContainerElement: HTMLElement;
  let fixture: ComponentFixture<ComponentThatProvidesWtf2Dialog>;

  beforeEach(fakeAsync(() => {
    TestBed.configureTestingModule({
      imports: [Wtf2DialogModule, DialogTestModule],
      declarations: [ComponentThatProvidesWtf2Dialog],
      providers: [
  ...
  beforeEach(inject([Wtf2Dialog], (d: Wtf2Dialog) => {
    parentDialog = d;

    fixture = TestBed.createComponent(ComponentThatProvidesWtf2Dialog);
  ...
  it('should close dialogs opened by a parent when calling closeAll on a child Wtf2Dialog',
    fakeAsync(() => {
      ...
      expect(overlayContainerElement.textContent!.trim())
          .toBe('', 'Expected closeAll on child Wtf2Dialog to close dialog opened by parent');
    }));

  it('should close dialogs opened by a child when calling closeAll on a parent Wtf2Dialog',
    fakeAsync(() => {
      ...
      expect(overlayContainerElement.textContent!.trim())
          .toBe('', 'Expected closeAll on parent Wtf2Dialog to close dialog opened by child');
    }));
...
describe('Wtf2Dialog with default options', () => {
  let dialog: Wtf2Dialog;
  let overlayContainer: OverlayContainer;
  let overlayContainerElement: HTMLElement;
  ...
  beforeEach(fakeAsync(() => {
    ...
    TestBed.configureTestingModule({
      imports: [Wtf2DialogModule, DialogTestModule],
      providers: [
        {provide: WTF2_DIALOG_DEFAULT_OPTIONS, useValue: defaultConfig},
      ],
    });
  ...
  beforeEach(inject([Wtf2Dialog, OverlayContainer],
    (d: Wtf2Dialog, oc: OverlayContainer) => {
      dialog = d;
      overlayContainer = oc;
      overlayContainerElement = oc.getContainerElement();
    }));
...
class ComponentWithTemplateRef {
  localValue: string;
  dialogRef: Wtf2DialogRef<any>;

  @ViewChild(TemplateRef, {static: false}) templateRef: TemplateRef<any>;

  setDialogRef(dialogRef: Wtf2DialogRef<any>): string {
...
class PizzaMsg {
  constructor(public dialogRef: Wtf2DialogRef<PizzaMsg>,
...
@Component({
  template: `
    <h1 wtf2-dialog-title>This is the title</h1>
    <wtf2-dialog-content>Lorem ipsum dolor sit amet.</wtf2-dialog-content>
    <wtf2-dialog-actions>
      <button wtf2-dialog-close>Close</button>
      <button class="close-with-true" [wtf2-dialog-close]="true">Close and return true</button>
      <button
        class="close-with-aria-label"
        aria-label="Best close button ever"
        [wtf2-dialog-close]="true"></button>
      <div wtf2-dialog-close>Should not close</div>
    </wtf2-dialog-actions>
  `
})
...
@Component({
  template: '',
  providers: [Wtf2Dialog]
})
class ComponentThatProvidesWtf2Dialog {
...
@NgModule({
  imports: [Wtf2DialogModule, NoopAnimationsModule],
