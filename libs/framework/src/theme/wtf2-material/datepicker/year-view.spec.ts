...
describe('Wtf2YearView', () => {
  ...
  describe('standard year view', () => {
    ...
    beforeEach(() => {
      ...
      let yearViewDebugElement = fixture.debugElement.query(By.directive(Wtf2YearView));
      yearViewNativeElement = yearViewDebugElement.nativeElement;
      testComponent = fixture.componentInstance;
    });

    it('has correct year label', () => {
      let labelEl = yearViewNativeElement.querySelector('.wtf2-calendar-body-label')!;
      expect(labelEl.innerHTML.trim()).toBe('2017');
    });

    it('has 12 months', () => {
      let cellEls = yearViewNativeElement.querySelectorAll('.wtf2-calendar-body-cell')!;
      expect(cellEls.length).toBe(12);
    });

    it('shows selected month if in same year', () => {
      let selectedEl = yearViewNativeElement.querySelector('.wtf2-calendar-body-selected')!;
    ...
    it('should mark active date', () => {
      ...
      expect(cellEls[0].classList).toContain('wtf2-calendar-body-active');
    });
  ...
  describe('year view with date filter', () => {
    ...
    beforeEach(() => {
      ...
      const yearViewDebugElement = fixture.debugElement.query(By.directive(Wtf2YearView));
      yearViewNativeElement = yearViewDebugElement.nativeElement;
    });

    it('should disable months with no enabled days', () => {
      const cells = yearViewNativeElement.querySelectorAll('.wtf2-calendar-body-cell');
...
@Component({
  template: `
    <wtf2-year-view [(activeDate)]="date" [(selected)]="selected"
                   (monthSelected)="selectedMonth=$event"></wtf2-year-view>`
})
class StandardYearView {
  ...
  @ViewChild(Wtf2YearView, {static: false}) yearView: Wtf2YearView<Date>;
}


@Component({
  template: `<wtf2-year-view [activeDate]="activeDate" [dateFilter]="dateFilter"></wtf2-year-view>`
})
