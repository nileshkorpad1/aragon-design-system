...
import {Wtf2CalendarBody, Wtf2CalendarCell, Wtf2CalendarCellCssClasses} from './calendar-body';
import {By} from '@angular/platform-browser';


describe('Wtf2CalendarBody', () => {
  beforeEach(async(() => {
  ...
  describe('standard calendar body', () => {
    ...
    function refreshElementLists() {
      ...
      labelEls = Array.from(calendarBodyNativeElement.querySelectorAll('.wtf2-calendar-body-label'));
      cellEls = Array.from(calendarBodyNativeElement.querySelectorAll('.wtf2-calendar-body-cell'));
    }
    ...
    beforeEach(() => {
      ...
      const calendarBodyDebugElement = fixture.debugElement.query(By.directive(Wtf2CalendarBody));
    ...
    it('highlights today', () => {
      const todayCell = calendarBodyNativeElement.querySelector('.wtf2-calendar-body-today')!;
      expect(todayCell).not.toBeNull();
      expect(todayCell.innerHTML.trim()).toBe('3');
    });

    it('highlights selected', () => {
      const selectedCell = calendarBodyNativeElement.querySelector('.wtf2-calendar-body-selected')!;
    ...
    it('places label in first row if space is available', () => {
      ...
      expect(rowEls[0].firstElementChild!.classList)
          .toContain('wtf2-calendar-body-label', 'first cell should be the label');
      expect(labelEls[0].getAttribute('colspan')).toBe('3');
    });

    it('cell should be selected on click', () => {
      const todayElement =
          calendarBodyNativeElement.querySelector('.wtf2-calendar-body-today') as HTMLElement;
      todayElement.click();
      fixture.detectChanges();

      expect(todayElement.classList)
          .toContain('wtf2-calendar-body-selected', 'today should be selected');
    });

    it('should mark active date', () => {
      expect((cellEls[10] as HTMLElement).innerText.trim()).toBe('11');
      expect(cellEls[10].classList).toContain('wtf2-calendar-body-active');
    });
...
@Component({
  template: `<table wtf2-calendar-body
                    [label]="label"
                    [rows]="rows"
...
function createCell(value: number, cellClasses?: Wtf2CalendarCellCssClasses) {
  return new Wtf2CalendarCell(value, `${value}`, `${value}-label`, true, cellClasses);
}
