`wtf2-icon` makes it easier to use _vector-based_ icons in your app.  This directive supports both
...
`Wtf2IconRegistry` is an injectable service that allows you to associate icon names with SVG URLs,
...
"home" as a home image. To use a ligature icon, put its text in the content of the `wtf2-icon`
component.

By default, `<wtf2-icon>` expects the
...
`Wtf2IconRegistry.registerFontClassAlias`.
...
itself or an alias registered with `Wtf2IconRegistry.registerFontClassAlias`), and set the `fontIcon`
...
explicitly set by calling `Wtf2IconRegistry.setDefaultFontSetClass`.

### SVG icons

When an `wtf2-icon` component displays an SVG icon, it does so by directly inlining the SVG content
...
change the color by setting the "color" style on the `wtf2-icon` element.

In order to prevent XSS vulnerabilities, any SVG URLs and HTML strings passed to the
`Wtf2IconRegistry` must be marked as trusted by using Angular's `DomSanitizer` service.
...
`addSvgIconLiteral` or `addSvgIconLiteralInNamespace` methods of `Wtf2IconRegistry`. After
...
`addSvgIconSetLiteral` or `addSvgIconSetLiteralInNamespace` methods of `Wtf2IconRegistry`.
...
screen-reader user. The user of `<wtf2-icon>` must provide additional inFormation pertaining to how
the icon is used. Based on the use-cases described below, `wtf2-icon` is marked as
...
#### Decorative icons
When the icon is purely cosmetic and conveys no real semantic meaning, the `<wtf2-icon>` element
...
* The `<wtf2-icon>` element should be a child of a `<button>` or `<a>` element.
...
1. Add a `<span>` as an adjacent sibling to the `<wtf2-icon>` element with text that conveys the same
...
you want to mirror an icon only in an RTL layout, you can use the `wtf2-icon-rtl-mirror` CSS class.

```html
<wtf2-icon class="wtf2-icon-rtl-mirror" svgIcon="thumb-up"></wtf2-icon>
```
