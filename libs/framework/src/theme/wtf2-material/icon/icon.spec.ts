...
import {Wtf2IconModule, WTF2_ICON_LOCATION} from './index';
import {Wtf2IconRegistry, getWtf2IconNoHttpProviderError} from './icon-registry';
...
describe('Wtf2Icon', () => {
  ...
  beforeEach(async(() => {
    ...
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule, Wtf2IconModule],
      declarations: [
        IconWithColor,
        IconWithLigature,
  ...
  let iconRegistry: Wtf2IconRegistry;
  let http: HttpTestingController;
  let sanitizer: DomSanitizer;

  beforeEach(inject([Wtf2IconRegistry, HttpTestingController, DomSanitizer],
    (mir: Wtf2IconRegistry, h: HttpTestingController, ds: DomSanitizer) => {
      iconRegistry = mir;
      http = h;
      sanitizer = ds;
    }));
  ...
  it('should include notranslate class by default', () => {
    ...
    const wtf2IconElement = fixture.debugElement.nativeElement.querySelector('wtf2-icon');
    expect(wtf2IconElement.classList.contains('notranslate'))
      .toBeTruthy('Expected the wtf2-icon element to include the notranslate class');
  });
  ...
  it('should apply class based on color attribute', () => {
    ...
    expect(sortedClassNames(wtf2IconElement))
        .toEqual(['wtf2-icon', 'wtf2-primary', 'material-icons', 'notranslate']);
  });
  ...
  it('should apply a class if there is no color', () => {
    ...
    expect(sortedClassNames(wtf2IconElement))
        .toEqual(['wtf2-icon', 'wtf2-icon-no-color', 'material-icons', 'notranslate']);
  });

  it('should mark wtf2-icon as aria-hidden by default', () => {
    const fixture = TestBed.createComponent(IconWithLigature);
    const iconElement = fixture.debugElement.nativeElement.querySelector('wtf2-icon');
    expect(iconElement.getAttribute('aria-hidden'))
      .toBe('true', 'Expected the wtf2-icon element has aria-hidden="true" by default');
  });
  ...
  it('should not override a user-provided aria-hidden attribute', () => {
    ...
    const iconElement = fixture.debugElement.nativeElement.querySelector('wtf2-icon');
    expect(iconElement.getAttribute('aria-hidden'))
      .toBe('false', 'Expected the wtf2-icon element has the user-provided aria-hidden value');
  });

  it('should apply inline styling', () => {
    const fixture = TestBed.createComponent(InlineIcon);
    const iconElement = fixture.debugElement.nativeElement.querySelector('wtf2-icon');
    expect(iconElement.classList.contains('wtf2-icon-inline'))
      .toBeFalsy('Expected the wtf2-icon element to not include the inline styling class');

    fixture.debugElement.componentInstance.inline = true;
    fixture.detectChanges();
    expect(iconElement.classList.contains('wtf2-icon-inline'))
      .toBeTruthy('Expected the wtf2-icon element to include the inline styling class');
  });
  ...
  describe('Ligature icons', () => {
    ...
    it('should use alternate icon font if set', () => {
      ...
      fixture.detectChanges();
      expect(sortedClassNames(wtf2IconElement))
          .toEqual(['wtf2-icon', 'wtf2-icon-no-color', 'myfont', 'notranslate']);
    });
    ...
    it('should not clear the text of a ligature icon if the svgIcon is bound to something falsy',
      () => {
        ...
        expect(wtf2IconElement.textContent.trim()).toBe('house');
      });
  ...
  describe('Icons from URLs', () => {
    ...
    it('should extract icon from SVG icon set', () => {
      ...
      expect(wtf2IconElement.childNodes.length).toBe(1);
      svgElement = verifyAndGetSingleSvgChild(wtf2IconElement);
    ...
    it('should clear the id attribute from the svg node', () => {
      ...
      const wtf2IconElement = fixture.debugElement.nativeElement.querySelector('wtf2-icon');
      const svgElement = verifyAndGetSingleSvgChild(wtf2IconElement);
    ...
    it('should remove the SVG element from the DOM when the binding is cleared', () => {
      ...
      const testComponent = fixture.componentInstance;
      const icon = fixture.debugElement.nativeElement.querySelector('wtf2-icon');
  ...
  describe('Icons from HTML string', () => {
    ...
    it('should add an extra string to the end of `style` tags inside SVG', fakeAsync(() => {
      ...
      fixture.detectChanges();
      const styleTag = fixture.nativeElement.querySelector('wtf2-icon svg style');
    ...
    it('should prepend the current path to attributes with `url()` references', fakeAsync(() => {
      ...
      fixture.componentInstance.iconName = 'fido';
      fixture.detectChanges();
      const circle = fixture.nativeElement.querySelector('wtf2-icon svg circle');
    ...
    it('should use latest path when prefixing the `url()` references', fakeAsync(() => {
      ...
      fixture.componentInstance.iconName = 'fido';
      fixture.detectChanges();
      let circle = fixture.nativeElement.querySelector('wtf2-icon svg circle');
      ...
      fixture.componentInstance.iconName = 'fido';
      fixture.detectChanges();
      circle = fixture.nativeElement.querySelector('wtf2-icon svg circle');
  ...
  describe('custom fonts', () => {
    it('should apply CSS classes for custom font and icon', () => {
      ...
      fixture.detectChanges();
      expect(sortedClassNames(wtf2IconElement))
          .toEqual(['font1', 'house', 'wtf2-icon', 'wtf2-icon-no-color', 'notranslate']);

      testComponent.fontSet = 'f2';
      testComponent.fontIcon = 'igloo';
      fixture.detectChanges();
      expect(sortedClassNames(wtf2IconElement))
          .toEqual(['f2', 'igloo', 'wtf2-icon', 'wtf2-icon-no-color', 'notranslate']);

      testComponent.fontSet = 'f3';
      testComponent.fontIcon = 'tent';
      fixture.detectChanges();
      expect(sortedClassNames(wtf2IconElement))
          .toEqual(['f3', 'wtf2-icon', 'wtf2-icon-no-color', 'notranslate', 'tent']);
    });
    ...
    it('should handle values with extraneous spaces being passed in to `fontSet`', () => {
      ...
      expect(sortedClassNames(wtf2IconElement))
          .toEqual(['font', 'wtf2-icon', 'wtf2-icon-no-color', 'notranslate']);
      ...
      expect(sortedClassNames(wtf2IconElement))
          .toEqual(['changed', 'wtf2-icon', 'wtf2-icon-no-color', 'notranslate']);
    });
    ...
    it('should handle values with extraneous spaces being passed in to `fontIcon`', () => {
      ...
      expect(sortedClassNames(wtf2IconElement))
        .toEqual(['font', 'wtf2-icon', 'wtf2-icon-no-color', 'material-icons', 'notranslate']);
      ...
      expect(sortedClassNames(wtf2IconElement))
        .toEqual(['changed', 'wtf2-icon', 'wtf2-icon-no-color', 'material-icons', 'notranslate']);
    });
...
describe('Wtf2Icon without HttpClientModule', () => {
  let iconRegistry: Wtf2IconRegistry;
  let sanitizer: DomSanitizer;

  beforeEach(async(() => {
    TestBed.configureTestingModule({
      imports: [Wtf2IconModule],
  ...
  beforeEach(inject([Wtf2IconRegistry, DomSanitizer], (mir: Wtf2IconRegistry, ds: DomSanitizer) => {
    iconRegistry = mir;
    sanitizer = ds;
  }));

  it('should throw an error when trying to load a remote icon', async() => {
    const expectedError = wrappedErrorMessage(getWtf2IconNoHttpProviderError());
...
@Component({template: `<wtf2-icon>{{iconName}}</wtf2-icon>`})
class IconWithLigature {
  iconName = '';
}

@Component({template: `<wtf2-icon [color]="iconColor">{{iconName}}</wtf2-icon>`})
class IconWithColor {
  iconName = '';
  iconColor = 'primary';
}

@Component({template: `<wtf2-icon [fontSet]="fontSet" [fontIcon]="fontIcon"></wtf2-icon>`})
class IconWithCustomFontCss {
  fontSet = '';
  fontIcon = '';
}

@Component({template: `<wtf2-icon [svgIcon]="iconName"></wtf2-icon>`})
class IconFromSvgName {
  iconName: string | undefined = '';
}

@Component({template: '<wtf2-icon aria-hidden="false">face</wtf2-icon>'})
class IconWithAriaHiddenFalse {}

@Component({template: `<wtf2-icon [svgIcon]="iconName" *ngIf="showIcon">{{iconName}}</wtf2-icon>`})
class IconWithBindingAndNgIf {
  iconName = 'fluffy';
  showIcon = true;
}

@Component({template: `<wtf2-icon [inline]="inline">{{iconName}}</wtf2-icon>`})
class InlineIcon {
  inline = false;
}

@Component({template: `<wtf2-icon [svgIcon]="iconName"><div>Hello</div></wtf2-icon>`})
class SvgIconWithUserContent {
  iconName: string | undefined = '';
}

@Component({template: '<wtf2-icon [svgIcon]="iconName">house</wtf2-icon>'})
