...
export class PerfectScrollbarDirective implements OnInit, OnDestroy, DoCheck, OnChanges {
  ...
  public scrollToElement(qs: string, offset?: number, speed?: number): void {
    const element1 = this.elementRef.nativeElement.querySelector(qs);
  ...
  public scrollToDocumentElement(qs: string, offset?: number, speed?: number): void {
    const element = document.querySelector(qs);
    if (element) {
      const elementPos = element.getBoundingClientRect();

      // const scrollerPos = this.elementRef.nativeElement.getBoundingClientRect();
      const scrollerPos = element.parentElement.getBoundingClientRect();

      if (element.parentElement.classList.contains('ps--active-x')) {
          const currentPos = element.parentElement['scrollLeft'];

          const position =  elementPos.left - scrollerPos.left + currentPos;

          this.animateScrollingDocument(
              'scrollLeft',
              position + (offset || 0),qs,
              speed,
          );
      }

      if (element.parentElement.classList.contains('ps--active-y')) {
          const currentPos = element.parentElement['scrollTop'];
  ...
  private animateScrollingDocument(target: string, value: number, qs: string,speed?: number): void {
    const element = document.querySelector(qs);
    if (this.animation) {
    ...
    if (!speed || typeof window === 'undefined') {
      element.parentElement[target] = value;
    } else if (value !== element.parentElement[target]) {
      let newValue = 0;
      let scrollCount = 0;

      let oldTimestamp = performance.now();
      let oldValue = element.parentElement[target];
      ...
      const step = (newTimestamp: number) => {
        ...
        if (element.parentElement[target] === oldValue) {
          if (scrollCount >= Math.PI) {
            this.animateScrollingDocument(target, value,qs, 0);
          } else {
            element.parentElement[target] = newValue;

            // On a zoomed out page the resulting offset may differ
            oldValue = element.parentElement[target];
