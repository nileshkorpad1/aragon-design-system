...
@Directive({
  ...
  selector: '[wtf2Summernote]',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => Wtf2SummernoteDirective),
    multi: true,
  }],
})
export class Wtf2SummernoteDirective implements ControlValueAccessor, OnInit, OnDestroy, OnChanges {
  @Input() set wtf2Summernote(options: any) {
  ...
  @Input() wtf2SummernoteDisabled: boolean;
  ...
  ngOnChanges(changes) {
    if (this._editorInitialized && changes) {
      if (changes.wtf2SummernoteDisabled && !changes.wtf2SummernoteDisabled.firstChange &&
        changes.wtf2SummernoteDisabled.currentValue !== changes.wtf2SummernoteDisabled.previousValue) {
        if (changes.wtf2SummernoteDisabled.currentValue) {
  ...
  private createEditor() {
    ...
    this.zone.runOutsideAngular(() => {
      ...
      if (this.wtf2SummernoteDisabled) {
        this._$element.summernote('disable');
      }
    });
