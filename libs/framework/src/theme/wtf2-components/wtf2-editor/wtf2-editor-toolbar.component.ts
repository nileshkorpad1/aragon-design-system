...
import { CustomClass, Font, Toolbar} from './config';
import * as Utils from './utils/editor.utils';

@Component({
  selector: 'wtf2-editor-toolbar',
  templateUrl: './wtf2-editor-toolbar.component.html',
  styleUrls: ['./wtf2-editor-toolbar.component.scss'],
})

export class Wtf2EditorToolbarComponent {
  id = '';
  foreground = 'black';
  background = 'grey';
  htmlMode = false;
  ...
  constructor(private _renderer: Renderer2,
              private editorService: Wtf2EditorService, @Inject(DOCUMENT) private _document: any) {
  }
  /**
    * Check toolbar availabe buttons
    * @param command string from toolbar buttons
    */
  canEnableToolbarOptions(value): boolean {
    return Utils.canEnableToolbarOptions(value, this.toolbar);
  }
  /**
      ...
      if (file.type.includes('image/')) {
        if(this.uploadUrl){
          ...
          reader.onload = (_event) => {
            if (typeof _event.target['result'] === 'string') {/*use csv*/
