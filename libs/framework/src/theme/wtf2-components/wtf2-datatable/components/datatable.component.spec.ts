...
import { Wtf2DatatableModule } from '../datatable.module';
...
class TestFixtureComponentWithCustomTemplatesComponent {
  rows: any[] = [];
  sorts: any[] = [];
...
describe('DatatableComponent With Custom Templates', () => {
  beforeEach(async(() => setupTest(TestFixtureComponentWithCustomTemplatesComponent)));
  ...
  it('should reflect changes to input bindings of `wtf2DatatableColumn`', () => {
    const initialRows = [
...
@Component({
  template: `
    <wtf2-datatable [rows]="rows" [sorts]="sorts">
      <wtf2DatatableColumn name="Id" prop="id">
        <ng-template let-column="column" wtf2-datatable-header-template>
          {{ column.name }}
        </ng-template>
        <ng-template let-row="row" wtf2-datatable-cell-template>
          {{ row.id }}
        </ng-template>
      </wtf2DatatableColumn>
      <wtf2DatatableColumn [prop]="columnTwoProp">
        <ng-template let-column="column" wtf2-datatable-header-template>
          {{ column.name }}
        </ng-template>
        <ng-template let-row="row" let-column="column" wtf2-datatable-cell-template>
          {{ row[column.prop] }}
        </ng-template>
      </wtf2DatatableColumn>
    </wtf2-datatable>
 ...
 * body of the wtf2-datatable component
 */
