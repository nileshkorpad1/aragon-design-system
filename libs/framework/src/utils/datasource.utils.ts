...
import { Wtf2Paginator, Wtf2Sort, PageEvent, Sort } from '../wtf2-material';
...
/** Creates an Observable stream of Sort objects from a Wtf2Sort component */
export function fromWtf2Sort(sort: Wtf2Sort): Observable<Sort> {
  return concat(
    defer(() => of({
      active: sort.active,
...
export function fromWtf2Paginators(pagers: QueryList<Wtf2Paginator>): Observable<PageEvent> {
  return pagers.changes.pipe(
    startWith(pagers),
    switchMap(_ =>
      merge(...pagers.map(p => fromWtf2Paginator(p))).pipe(
...
/** Creates an Observable stream of PageEvent objects from a Wtf2Paginator component */
export function fromWtf2Paginator(pager: Wtf2Paginator): Observable<PageEvent> {
  return concat(
    defer(() => of({
