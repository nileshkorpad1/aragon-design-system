...
import { WTF2_AUTH_OPTIONS, WTF2_AUTH_USER_OPTIONS, WTF2_AUTH_STRATEGIES, WTF2_AUTH_TOKENS } from '../auth.options';
import { Wtf2AuthService } from './auth.service';
import { Wtf2DummyAuthStrategy } from '../strategies';
import { wtf2StrategiesFactory, wtf2OptionsFactory } from '../auth.module';
import { Wtf2AuthResult } from './auth-result';
import { Wtf2TokenService } from './token/token.service';
import { Wtf2AuthSimpleToken, wtf2AuthCreateToken, Wtf2AuthJWTToken } from './token/token';
import { Wtf2TokenLocalStorage, Wtf2TokenStorage } from './token/token-storage';
import { WTF2_AUTH_FALLBACK_TOKEN, Wtf2AuthTokenParceler } from './token/token-parceler';
...
describe('auth-service', () => {
  ...
  const testToken = wtf2AuthCreateToken(Wtf2AuthSimpleToken, testTokenValue, ownerStrategyName);
  const invalidToken = wtf2AuthCreateToken(Wtf2AuthSimpleToken, testTokenValue, ownerStrategyName);
  const emptyToken = wtf2AuthCreateToken(Wtf2AuthSimpleToken, null, null);

  const failResult = new Wtf2AuthResult(false,
    resp401,
    null,
    ['Something went wrong.']);

  const successResult = new Wtf2AuthResult(true,
    resp200,
    '/',
    [],
  ...
  const successLogoutResult = new Wtf2AuthResult(true,
    resp200,
    '/',
    [],
    ['Successfully logged out.']);

  const successResetPasswordResult = new Wtf2AuthResult(true,
    resp200,
    '/',
    [],
    ['Successfully reset password.']);

  const successRequestPasswordResult = new Wtf2AuthResult(true,
    resp200,
    '/',
    [],
    ['Successfully requested password.']);

  const successRefreshTokenResult = new Wtf2AuthResult(true,
    resp200,
    null,
    [],
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [
        { provide: WTF2_AUTH_OPTIONS, useValue: {} },
  ...
  it('get test token before set', () => {
      ...
      authService.getToken().subscribe((val: Wtf2AuthSimpleToken) => {
  ...
  it('onTokenChange return correct stream and gets test token', (done) => {
      ...
      authService.onTokenChange()
        .pipe(first())
        .subscribe((token: Wtf2AuthSimpleToken) => {
          expect(spy).toHaveBeenCalled();
  ...
  it('authenticate failed', (done) => {
      ...
      authService.authenticate(ownerStrategyName).subscribe((authRes: Wtf2AuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('register failed', (done) => {
      ...
      authService.register(ownerStrategyName).subscribe((authRes: Wtf2AuthResult) => {
        expect(spy).toHaveBeenCalled();
        expect(authRes.isFailure()).toBeTruthy();
        expect(authRes.isSuccess()).toBeFalsy();
        expect(authRes.getMessages()).toEqual([]);
  ...
  it('logout failed', (done) => {
      ...
      authService.logout(ownerStrategyName).subscribe((authRes: Wtf2AuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('requestPassword failed', (done) => {
      ...
      authService.requestPassword(ownerStrategyName).subscribe((authRes: Wtf2AuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('resetPassword failed', (done) => {
      ...
      authService.resetPassword(ownerStrategyName).subscribe((authRes: Wtf2AuthResult) => {
        expect(spy).toHaveBeenCalled();
  ...
  it('refreshToken failed', (done) => {
      ...
      authService.refreshToken(ownerStrategyName).subscribe((authRes: Wtf2AuthResult) => {
        expect(spy).toHaveBeenCalled();
        expect(authRes.isFailure()).toBeTruthy();
        expect(authRes.isSuccess()).toBeFalsy();
        expect(authRes.getMessages()).toEqual([]);
