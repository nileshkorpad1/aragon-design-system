...
export abstract class Wtf2AuthToken {
  ...
  getName(): string {
    return (this.constructor as Wtf2AuthTokenClass).NAME;
  }
...
export class Wtf2AuthTokenNotFoundError extends Error {
...
export class Wtf2AuthIllegalTokenError extends Error {
  constructor(message: string) {
    super(message);
    Object.setPrototypeOf(this, new.target.prototype);
  }
...
export class Wtf2AuthEmptyTokenError extends Wtf2AuthIllegalTokenError {
  constructor(message: string) {
    super(message);
    Object.setPrototypeOf(this, new.target.prototype);
  }
...
export class Wtf2AuthIllegalJWTTokenError extends Wtf2AuthIllegalTokenError {
  constructor(message: string) {
    super(message);
    Object.setPrototypeOf(this, new.target.prototype);
  }
...
export interface Wtf2AuthRefreshableToken {
  getRefreshToken(): string;
  setRefreshToken(refreshToken: string);
}

export interface Wtf2AuthTokenClass<T = Wtf2AuthToken> {
  NAME: string;
  new (raw: any, strategyName: string, expDate?: Date): T;
}

export function wtf2AuthCreateToken<T extends Wtf2AuthToken>(tokenClass: Wtf2AuthTokenClass<T>,
                                  token: any,
...
export function decodeJwtPayload(payload: string): any {
  ...
  if (payload.length === 0) {
    throw new Wtf2AuthEmptyTokenError('Cannot extract from an empty payload.');
  }

  const parts = payload.split('.');

  if (parts.length !== 3) {
    throw new Wtf2AuthIllegalJWTTokenError(
 ...
 */
export class Wtf2AuthSimpleToken extends Wtf2AuthToken {
  ...
  constructor(protected readonly token: any,
    ...
    try {
      ...
      if (!(err instanceof Wtf2AuthTokenNotFoundError)) {
 ...
 */
export class Wtf2AuthJWTToken extends Wtf2AuthSimpleToken {
  ...
  protected parsePayload(): void {
    if (!this.token) {
      throw new Wtf2AuthTokenNotFoundError('Token not found. ');
    }
 ...
 */
export class Wtf2AuthOAuth2Token extends Wtf2AuthSimpleToken {
  ...
  protected parsePayload(): void {
    if (!this.token) {
      ...
      if (!Object.keys(this.token).length) {
        throw new Wtf2AuthEmptyTokenError('Cannot extract payload from an empty token.');
      }
    }
 ...
 */
export class Wtf2AuthOAuth2JWTToken extends Wtf2AuthOAuth2Token {
  ...
  protected parseAccessTokenPayload(): any {
    ...
    if (!accessToken) {
      throw new Wtf2AuthTokenNotFoundError('access_token key not found.');
    }
