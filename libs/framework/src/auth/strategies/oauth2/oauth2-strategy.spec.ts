...
import { Wtf2OAuth2AuthStrategy } from './oauth2-strategy';
import { Wtf2OAuth2ClientAuthMethod, Wtf2OAuth2GrantType, Wtf2OAuth2ResponseType } from './oauth2-strategy.options';
import { Wtf2AuthResult, wtf2AuthCreateToken, Wtf2AuthOAuth2Token } from '../../services';
...
describe('oauth2-auth-strategy', () => {
  ...
  let strategy: Wtf2OAuth2AuthStrategy;
  ...
  const successToken = wtf2AuthCreateToken(Wtf2AuthOAuth2Token, tokenSuccessResponse, 'strategy') as Wtf2AuthOAuth2Token;
  // tslint:disable-next-line
  const refreshedToken = wtf2AuthCreateToken(Wtf2AuthOAuth2Token, refreshedTokenPayload, 'strategy') as Wtf2AuthOAuth2Token;
  const refreshedTokenWithRefreshToken = wtf2AuthCreateToken(Wtf2AuthOAuth2Token, refreshedTokenResponse, 'strategy') as Wtf2AuthOAuth2Token;
  ...
  beforeEach(async(inject(
    [Wtf2OAuth2AuthStrategy, HttpTestingController],
    (_strategy, _httpMock) => {
      strategy = _strategy;
      httpMock = _httpMock;
  ...
  describe('out of the box: type CODE', () => {
    ...
    it('handle success redirect and sends correct token request', (done: DoneFn) => {
      ...
      strategy.authenticate()
        .subscribe((result: Wtf2AuthResult) => {
      ...
      httpMock.expectOne(
        req => req.url === 'http://example.com/token'
            && req.body['grant_type'] === Wtf2OAuth2GrantType.AUTHORIZATION_CODE
            && req.body['code'] === 'code'
    ...
    it('handle refresh token with basic client auth', (done: DoneFn) => {
      strategy.setOptions({
        ...
        clientAuthMethod: Wtf2OAuth2ClientAuthMethod.BASIC,
      });
      ...
      httpMock.expectOne(
        req => req.url === 'http://example.com/token'
          ...
          && req.body['grant_type'] === Wtf2OAuth2GrantType.REFRESH_TOKEN
    ...
    it('handle refresh token with requestBody client auth', (done: DoneFn) => {
      strategy.setOptions({
        ... basicOptions,
        clientSecret: 'clientSecret',
        clientAuthMethod: Wtf2OAuth2ClientAuthMethod.REQUEST_BODY,
      });
      strategy.refreshToken(successToken)
        .subscribe((result: Wtf2AuthResult) => {
          expect(result).toBeTruthy();
  ...
  describe('configured: type TOKEN', () => {
    ...
    const basicOptions = {
      ...
      clientId: 'clientId',
      authorize: {
        responseType: Wtf2OAuth2ResponseType.TOKEN,
      },
    };
    ...
    it('handle success redirect back with token', (done: DoneFn) => {
      ...
      strategy.authenticate()
        .subscribe((result: Wtf2AuthResult) => {
          ...
          expect(result.isFailure()).toBe(false);
          // tslint:disable-next-line
          expect(result.getToken().getValue()).toEqual(wtf2AuthCreateToken(Wtf2AuthOAuth2Token, token, 'strategy').getValue());
          expect(result.getMessages()).toEqual(successMessages);
          expect(result.getErrors()).toEqual([]); // no error message, response is success
          expect(result.getRedirect()).toEqual('/');
          done();
  ...
  describe('configured: additionnal param: token, grant_type:password', () => {
    ...
    const basicOptions = {
      ...
      clientId: 'clientId',
      token: {
        grantType: Wtf2OAuth2GrantType.PASSWORD,
        endpoint: 'token',
        scope: scope,
      },
    };
    ...
    it('handle success login with NO client auth', (done: DoneFn) => {
      ...
      httpMock.expectOne(
        req => req.url === 'http://example.com/token'
          && req.body['grant_type'] === Wtf2OAuth2GrantType.PASSWORD
